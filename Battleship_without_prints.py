        ...
        for event in pygame.event.get():
            if event.type == 5:
                ...
                if top_corner(event.pos) in how_to.cell_cover():
                    how_to.onclick()
                if top_corner(event.pos) in qbutton_2.cell_cover():
                    qbutton_2.onclick()
            if event.type == 12:
                qsequence()
            pygame.display.update()
...
def How_to_play():
    ...
    message_to_screen(100, 120, "1. The two boards displayed on the screen are yours and the computer (left to right respectively.)", white)
    message_to_screen(100, 170, "2. Select the ship tokens one by one and align them using arrow keys.", white)
    message_to_screen(100, 220, "3. Click the box on your board to place the ship token.", white)
    message_to_screen(100, 270, "4. Goal: To destroy all the ships of the AI by correctly guessing their location on its board.", white)
    message_to_screen(100, 370, "HAPPY BRAINSTORMING!", white)
    while True:
        for event in pygame.event.get():
            if event.type == 5:
...
qbutton_2 = button(12*x, 9*y, 4*x, 2*y, qsequence, quit_2_button_image, False)
...
buttons = [aircraft_carrier_button, cruiser_button, submarine_button, destroyer_button, how_to, back_to, play1, play2, replay, qbutton, qbutton_2]
